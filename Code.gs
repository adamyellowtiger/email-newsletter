...
const AI_FACTS = [
  'Transformers use attention to estimate which earlier tokens matter most for predicting or generating the next token.',
  'A model announcement is strongest when it links to a system card, technical report, benchmark details, or reproducible demo.',
  'Open-weight models are not automatically open source; license terms still decide what people can legally do with them.',
  'A benchmark score is most useful when the prompt format, evaluation set, and contamination controls are disclosed.',
  'Latency often matters as much as raw model quality for real products, especially agents and voice interfaces.',
  'Fine-tuning usually changes behavior within a task; retrieval usually changes what information the model can reference.',
  'AI safety evaluations are snapshots, not permanent guarantees, because model behavior changes with prompts, tools, and deployment context.',
  'Multimodal systems combine separate signals such as text, image, audio, or video into one reasoning or generation workflow.',
  'A small model can beat a frontier model on a narrow workflow when the data, prompts, tools, and evaluation loop are strong.',
  'The most useful AI product launches usually include pricing, limits, model/version names, and migration notes.'
];
...
function composeIssue_(issueDate, candidates) {
  ...
  const usedKeys = {};
  const topStories = selected.slice(0, 3);
  markItemsUsed_(topStories, usedKeys);
  const tools = selected.filter(function (item) {
    return !usedKeys[dedupeKey_(item)] && itemMatches_(item, ['tool', 'api', 'developer', 'launch', 'release', 'agent', 'open source', 'model']);
  }).slice(0, 3);
  markItemsUsed_(tools, usedKeys);
  const research = selected.filter(function (item) {
    return !usedKeys[dedupeKey_(item)] && (String(item.category).toLowerCase() === 'research' || itemMatches_(item, ['paper', 'benchmark', 'arxiv', 'eval', 'research']));
  }).slice(0, 3);
...
function renderIssueHtml_(issueDate, topStories, tools, research, selected, fact) {
  ...
  const html = [
    ...
    '<p style="margin:0 0 22px 0;font-size:15px;line-height:1.5;color:#374151;">A concise daily scan of AI news, launches, research, and useful facts for builders and curious readers.</p>',
    renderSectionHtml_('Top 3', topStories),
    renderSectionHtml_('New Tools', tools),
    renderSectionHtml_('Research', research),
    '<h2 style="margin:26px 0 10px 0;font-size:18px;color:#111827;">One Useful Fact</h2>',
    '<p style="margin:0 0 18px 0;font-size:15px;line-height:1.6;color:#374151;">' + escapeHtml_(fact) + '</p>',
    renderSectionHtml_('Links', selected),
...
function renderSectionHtml_(title, items) {
  ...
  const blocks = items.map(function (item) {
    return [
      ...
      '<p style="margin:0 0 6px 0;font-size:14px;line-height:1.55;color:#374151;">' + escapeHtml_(item.summary) + '</p>',
...
function renderIssueText_(issueDate, topStories, tools, research, selected, fact) {
  ...
  return [
    ...
    '',
    'A concise daily scan of AI news, launches, research, and useful facts for builders and curious readers.',
    '',
    renderSectionText_('Top 3', topStories),
    renderSectionText_('New Tools', tools),
    renderSectionText_('Research', research),
    'One Useful Fact',
    fact,
    '',
    renderSectionText_('Links', selected),
    footerReason,
...
function whyItMatters_(item) {
  ...
  if (text.indexOf('api') !== -1 || text.indexOf('developer') !== -1) {
    return 'Builders may get new capabilities, pricing implications, or migration work to consider.';
  }
  if (text.indexOf('agent') !== -1) {
    return 'Agent updates can change how teams automate multi-step work and connect models to tools.';
  }
  if (text.indexOf('open source') !== -1 || text.indexOf('open-weight') !== -1) {
    return 'Open releases can shift what small teams can test, customize, and run without a closed API dependency.';
  }
  if (text.indexOf('benchmark') !== -1 || text.indexOf('eval') !== -1 || text.indexOf('paper') !== -1 || String(item.category).toLowerCase() === 'research') {
    return 'Research signals where model capabilities, evaluation methods, and product ideas may move next.';
  }
  if (text.indexOf('safety') !== -1 || text.indexOf('policy') !== -1 || text.indexOf('regulation') !== -1) {
    return 'Governance and safety updates affect what products can responsibly ship and how users may trust them.';
  }
  if (text.indexOf('funding') !== -1 || text.indexOf('acquisition') !== -1 || text.indexOf('enterprise') !== -1) {
    return 'Business moves reveal where AI infrastructure, adoption, and competitive pressure are concentrating.';
  }
  return 'This is a notable signal in the AI ecosystem and is worth tracking for follow-up.';
}
